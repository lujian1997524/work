...
interface MaterialToastHelper {
  ...
  dimensionUpdated: (dimensions: string, quantity: number) => void;
  dimensionDeleted: (dimensions: string, quantity: number) => void;
...
export const materialToastHelper: MaterialToastHelper = {
  ...
  dimensionUpdated: (dimensions: string, quantity: number) => {
    playMaterialSound('success');
    showMaterialNotification(
      'success',
      '尺寸更新',
      `尺寸${dimensions}已更新，数量${quantity}张`,
      3000
    );
  },
  
  dimensionDeleted: (dimensions: string, quantity: number) => {
    playMaterialSound('info');
    showMaterialNotification(
      'info',
      '尺寸删除',
      `尺寸${dimensions}已删除，原数量${quantity}张`,
      3000
    );
  },
