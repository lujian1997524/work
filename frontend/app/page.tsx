...
import { MaterialInventoryManager } from '@/components/materials/MaterialInventoryManager';
    ...
    if (result.jumpTo) {
      ...
      switch (jumpTo.type) {
        case 'worker':
          ...
          if (jumpTo.filters?.workerId) {
            setWorkerIdFilter(jumpTo.filters.workerId);
          ...
          console.log(`🔗 跳转到工人: ${result.title}`);
          break;
          
        case 'project':
          // 项目详情 - 跳转到对应项目视图
          const projectStatus = result.metadata?.status || 'active';
          ...
          console.log(`🔗 跳转到项目: ${result.title} (状态: ${projectStatus})`);
          break;
          
        case 'employee':
          // 员工考勤 - 跳转到考勤管理页面
          setViewType('attendance');
          // 可以进一步设置员工筛选，如果考勤组件支持
          console.log(`🔗 跳转到员工考勤: ${result.title}`);
          break;
        ...
        case 'thickness_spec':
          ...
          if (jumpTo.filters?.thicknessFilter) {
            setMaterialThicknessFilter(jumpTo.filters.thicknessFilter);
          } else if (result.metadata) {
            // 根据元数据构建筛选条件
            const materialType = result.metadata.materialType;
            const thickness = result.metadata.thickness;
            setMaterialTypeFilter(materialType === '碳板' ? 'carbon' : materialType === '不锈钢' ? 'stainless' : 'special');
            setMaterialThicknessFilter(`${materialType}_${thickness}`);
          }
          // 清除工人筛选
          setWorkerIdFilter(null);
          setWorkerNameFilter('');
          console.log(`🔗 跳转到板材规格: ${result.title}`);
          break;
        ...
        case 'drawing':
          ...
          if (jumpTo.projectId) {
            // 如果图纸关联项目，跳转到项目视图
            const projectStatus = result.relatedData?.project?.status || 'active';
            setViewType(projectStatus === 'completed' ? 'completed' : 'active');
            setSelectedProjectId(jumpTo.projectId);
          } else {
            // 否则跳转到图纸库
            setViewType('drawings');
            if (result.metadata?.category) {
              setDrawingCategory(result.metadata.category);
            }
          }
          console.log(`🔗 跳转到图纸: ${result.title}`);
          break;
          
        case 'attendance_record':
          // 考勤记录详情 - 跳转到考勤管理
          setViewType('attendance');
          console.log(`🔗 跳转到考勤记录: ${result.title}`);
          break;
          
        default:
          console.warn(`❌ 未处理的jumpTo类型: ${jumpTo.type}`, result);
          break;
      }
    } else {
      // 兼容旧格式 - 降级处理
      console.warn('⚠️ 搜索结果缺少jumpTo信息，使用降级导航', result);
      ...
      switch (result.type) {
        ...
        default:
          console.warn(`❌ 未处理的搜索结果类型: ${result.type}`, result);
          break;
      }
    }
  };
