...
export const DimensionManager: React.FC<DimensionManagerProps> = ({
  ...
  // 当 initialDimensions 发生变化时，同步更新本地状态
  useEffect(() => {
  ...
  const deleteDimension = async (dimensionId: number) => {
    if (!confirm('确定要删除这个尺寸记录吗？')) return;

    const targetDimension = dimensions.find(d => d.id === dimensionId);
    if (!targetDimension) {
      materialToastHelper.warning('该尺寸记录已不存在');
      return;
    }

    const dimensionText = `${targetDimension.width || 0}×${targetDimension.height || 0}mm`;
    setLoading(true);
    ...
    try {
      ...
      // 无论API成功还是404，都从界面删除
      const newDimensions = dimensions.filter(dim => dim.id !== dimensionId);
      ...
      if (response.ok) {
        ...
        materialToastHelper.warning(`板材尺寸 ${dimensionText} 已不存在，已从列表中移除`);
      } else {
...
const DimensionEditItem: React.FC<DimensionEditItemProps> = ({
  ...
  const [formData, setFormData] = useState<DimensionFormData>({
    width: (dimension.width ?? 0).toString(),
    height: (dimension.height ?? 0).toString(),
    quantity: (dimension.quantity ?? 0).toString(),
