...
export const ActiveProjectsCardView: React.FC<ActiveProjectsCardViewProps> = ({
  ...
  const handleExportToExcel = async () => {
    try {
      console.log('📊 开始导出Excel报表...');
      console.log(`📊 当前项目数据: ${projects.length} 个项目`, projects);
      
      // 筛选活跃项目
      const activeProjects = projects.filter(p => 
        p.status === 'pending' || p.status === 'in_progress'
      );
      console.log(`📊 活跃项目: ${activeProjects.length} 个`, activeProjects);
      ...
      // 调用导出函数，传递项目数据
      await exportActiveProjectsReport(activeProjects);
      console.log('📊 Excel导出成功');
      
      toast.addToast({
        type: 'success',
        message: `活跃项目详细报表已导出 (${activeProjects.length} 个项目)`
      });
    } catch (error) {
  ...
  const fetchAnnouncements = async () => {
    try {
      ...
      console.log('📢 API响应状态:', response.ok);
      console.log('📢 API响应数据:', result);
      
      if (response.ok) {
        const announcementList = result.announcements || [];
        console.log(`📢 设置公告列表，共 ${announcementList.length} 条公告:`, announcementList);
        setAnnouncements(announcementList);
      } else {
        console.error('📢 获取公告失败，响应不成功:', result);
      }
    } catch (error) {
  ...
  // 队列排序相关函数
  const fetchQueueProjects = async (projectsList?: ActiveProject[]) => {
    try {
      setQueueSortLoading(true);
      // 使用传入的项目列表，或者当前的projects状态
      const sourceProjects = projectsList || projects;
      // 获取活跃项目列表用于排序
      const activeProjects = sourceProjects.filter(p => 
        p.status === 'pending' || p.status === 'in_progress'
      );
      console.log(`📋 筛选出 ${activeProjects.length} 个活跃项目用于队列排序`);
  ...
  const handleSaveQueueOrder = async () => {
    try {
      ...
      if (response.ok) {
        ...
        // 重要：保存成功后刷新数据
        console.log('🔄 队列排序保存成功，正在刷新数据...');
        
        // 1. 刷新主项目列表（从后端获取最新的sortOrder）
        await fetchProjects();
        console.log('✅ 主项目列表已刷新');
        
        // 2. 直接使用 useProjectStore 中最新的 projects 数据
        // 从 store 中获取最新的项目数据
        const latestProjects = useProjectStore.getState().projects;
        console.log(`📊 获取到 ${latestProjects.length} 个最新项目数据`);
        
        // 3. 基于最新数据重新获取队列项目
        await fetchQueueProjects(latestProjects);
        console.log('✅ 队列项目列表已刷新');
        
        // 4. 显示更新成功的提示，2秒后自动关闭模态框
        setTimeout(() => {
