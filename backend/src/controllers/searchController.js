...
const searchWorkers = async (query, limit) => {
  const { Worker, Department } = require('../models');
  ...
  const workers = await Worker.findAll({
    where: {
      [Op.or]: [
        ...
        // 通过关联的部门名称搜索
        { '$departmentInfo.name$': { [Op.like]: `%${query}%` } }
      ]
    },
    include: [
      {
  ...
  return workers.map(worker => ({
    ...
    type: 'workers',
    title: worker.name,
    subtitle: `${worker.departmentInfo?.name || '未分配部门'} | ${worker.position || '未知职位'}`,
